...
def remove_datatype():
    ...
    try:
        ...
        datatype_select_menu = ttk.Combobox(gui, textvariable=optionsmenu_dict['start'], values=list(relevantinfo.keys()), state='readonly')
        datatype_select_menu.bind("<<ComboboxSelected>>", add_box1)
...
def remove_last(value):
    ...
    try:
        ...
        datatype_select_menu = ttk.Combobox(gui, textvariable=optionsmenu_dict[value], values=list(newrelevantinfo.keys()), state='readonly')
        datatype_select_menu.bind("<<ComboboxSelected>>", add_box1)
...
def add_box1(value):
    ...
    try:
        del newrelevantinfo[value.widget.get()]
    except ValueError:
        ''
    ...
    if lastoptionsmenuval == '':
        remove_last(value.widget.get())
    else:
        entry_dict[value.widget.get()] = tk.StringVar()

        datatype_entry_1_field = tk.Entry(gui, textvariable=entry_dict[value.widget.get()])
        datatype_entry_1_field.grid(row=6 + len(entry_dict), column=1, columnspan=4, sticky='nesw')
        datatype_entry_1_field.bind("<Button-3>", RightClicker)
        removelist.append(datatype_entry_1_field)

        if (value.widget.get() == 'hasScored') or (value.widget.get() == 'isOut'):
            datatype_entry_1_field.insert(tk.END, 'True')

        try:
            optionsmenu_dict[value.widget.get()] = tk.StringVar()
            datatype_select_menu = ttk.Combobox(gui, textvariable=optionsmenu_dict[value.widget.get()], values=list(newrelevantinfo.keys()), state='readonly')
            datatype_select_menu.bind("<<ComboboxSelected>>", add_box1)
            datatype_select_menu.grid(row=7 + len(entry_dict), column=0, sticky='nesw')
            removelist.append(datatype_select_menu)
        except TypeError:
...
def save_all():
    ...
    with open(currentpath + '/Data/Filetitles.pkl', 'rb') as f:
        titledict = pickle.load(f)

    texttitle = titledict[filename]
...
results_field.bind("<Button-3>", RightClicker)
...
currentsentence_field.bind("<Button-3>", RightClicker)
...
save_button = tk.Button(gui, text='Opslaan', command=save_all, font=('TkDefaultFont', 9,'bold'))
...
save_field.grid(row=31, column=0, columnspan=3, sticky='nesw')
save_field.delete(0, tk.END)
