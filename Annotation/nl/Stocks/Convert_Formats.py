            ...
            for paragraph in text:
                        ...
                        if (combineddictkey != 'sentenceidx') and (combineddictkey != 'filename') and (combineddictkey != 'sentence') and (combineddictkey != 'title') and (combineddictkey != 'textidx') and (combineddictkey != 'paragraphidx') and (combineddictkey != 'datasetidx'):
                            ...
                            if annotationtype == 'MyTool':
                                if newkey not in questiondict:
                                    questionid = str(sentence['sentenceidx'][0]) + '/' + str(sentence['sentenceidx'][1]) + '/' + str(sentence['sentenceidx'][2]) + '_' + newkey
                ...
                for information in relevantinfo:
                    ...
                    if newinfo not in questiondict:
                        try:
                            questionid = str(paragraph[0]['sentenceidx'][0]) + '/' + str(paragraph[0]['sentenceidx'][1]) + '/' + str(paragraph[0]['sentenceidx'][2]) + '_' + newinfo
                            questiondict.update({newinfo: {'question': newinfo, 'id': questionid, 'answers': [], 'is_impossible': True}})
                        except TypeError:
                            questionid = str(paragraph[0]['datasetidx']) + '/' + str(paragraph[0]['textidx']) + '/' + str(paragraph[0]['paragraphidx']) + '_' + newinfo
                            questiondict.update({newinfo: {'question': newinfo, 'id': questionid, 'answers': [], 'is_impossible': True}})
...
def phrase_dict(newsentence, splitsentence):
    if not os.path.isfile(currentpath + '/Data/Phrase_Dict.pkl'):
        phrasedict = {}
        with open(currentpath + '/Data/Phrase_Dict.pkl', 'wb') as f:
            pickle.dump(phrasedict, f)
    else:
        with open(currentpath + '/Data/Phrase_Dict.pkl', 'rb') as f:
            phrasedict = pickle.load(f)
    for idx, val in enumerate(newsentence):
        if ('[' in val) and ('=' in val) and (']' in val):
            phrasekey = newsentence[idx]
            phrasevalue = splitsentence[idx].lower()
            if phrasekey not in phrasedict:
                phrasedict.update({phrasekey: [phrasevalue]})
            if phrasekey in phrasedict:
                if phrasevalue not in phrasedict[phrasekey]:
                    phrasedict[phrasekey].append(phrasevalue)
...
def lexicalizesentence(sentence, referencedict, annotationtype):
    ...
    phrase_dict(newsentence, splitsentence)
...
def webnlg(alldatasets, language):
    ...
    with open(currentpath + '/Data/Phrase_Dict.pkl', 'rb') as f:
        ...
        # Write JSON file
    with open(currentpath + '/Data/Phrase_Dict.json', 'w') as outfile:
        json.dump(alldatasets, outfile, indent=4, separators=(',', ': '))
