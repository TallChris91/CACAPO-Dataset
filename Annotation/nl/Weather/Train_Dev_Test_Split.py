...
dutchweather = Convert_Prodigy(currentpath + '/Data/DutchWeatherAll.jsonl', 'nl')

trainsample = np.random.choice(np.arange(len(dutchweather)), int(round((len(dutchweather) / 100) * 76.5, 0)), replace=False)
newrestsample = []
restsample = list(np.arange(len(dutchweather)))
for i in restsample:
    if i not in trainsample:
        newrestsample.append(i)

devsample = np.random.choice(newrestsample, int(round((len(dutchweather) / 100) * 8.5, 0)), replace=False)
...
newdatalisttrain = np.array(dutchweather)[trainsample]
newdatalistdev = np.array(dutchweather)[devsample]
newdatalisttest = np.array(dutchweather)[testsample]
