...
import spacy
nlnlp = spacy.load("nl_core_news_md")
...
englishweather = Convert_Prodigy(currentpath + '/Data/EnglishWeatherAll.jsonl', 'en')

print(len(englishweather))
...
filenamesconcatenated = GetFileNames(currentpath + '/Data/EnglishWeatherAll.jsonl')

trainsample = np.random.choice(np.arange(len(englishweather)), int(round((len(englishweather) / 100) * 76.5, 0)), replace=False)
newrestsample = []
restsample = list(np.arange(len(englishweather)))
for i in restsample:
    if i not in trainsample:
        newrestsample.append(i)

devsample = np.random.choice(newrestsample, int(round((len(englishweather) / 100) * 8.5, 0)), replace=False)
...
newdatalisttrain = np.array(englishweather)[trainsample]
newdatalistdev = np.array(englishweather)[devsample]
newdatalisttest = np.array(englishweather)[testsample]
