...
def squaddataset(alldatasets):
    ...
    for datasetidx, dataset in enumerate(alldatasets):
        ...
        for text in dataset:
            ...
            for paragraph in text:
                ...
                for sentence in paragraph:
                    ...
                    paragraphdict = {'qas': [], 'context': currentsentence}
                    questiondict = {}
                    ...
                    for combineddictkey in sentence:
                        if (combineddictkey != 'sentenceidx') and (combineddictkey != 'filename') and (combineddictkey != 'sentence') and (combineddictkey != 'title') and (combineddictkey != 'textidx') and (combineddictkey != 'paragraphidx') and (combineddictkey != 'datasetidx'):
                            ...
                            if annotationtype == 'Prodigy':
                                ...
                                newkey = re.sub(r'\d+', '', combineddictkey)
                                if ';;' in sentence[combineddictkey]:
                                    ...
                                    for content in newsplit:
                                        ...
                                        for actualidx in startindices:
                                            ...
                                            if (newkey in questiondict) and (ad in questiondict[newkey]['answers']):
                                                continue
                                            else:
                                                answerdict.append(ad)
                                else:
                                    ...
                                    if len(startindices) > 0:
                                        ...
                                        if (newkey in questiondict) and (answerdict in questiondict[newkey]['answers']):
                                            pass
                                        else:
                                            answerdict.append(ad)

                            if annotationtype == 'MyTool':
                                if newkey not in questiondict:
                                    questionid = str(sentence['sentenceidx'][0]) + '/' + str(sentence['sentenceidx'][1]) + '/' + str(sentence['sentenceidx'][2]) + '/' + str(sentence['sentenceidx'][3]) + '_' + newkey
                                    questiondict.update({newkey: {'question': newkey, 'id': questionid, 'answers': answerdict, 'is_impossible': False}})
                                else:
                                    questiondict[newkey]['answers'] = questiondict[newkey]['answers'] + answerdict
                            elif annotationtype == 'Prodigy':
                                if combineddictkey not in questiondict:
                                    questionid = str(sentence['datasetidx']) + '/' + str(sentence['textidx']) + '/' + str(sentence['paragraphidx']) + '/' + str(sentence['sentenceidx']) + '_' + combineddictkey
                    ...
                    for information in relevantinfo:
                        newinfo = re.sub(r'\d+', '', information)
                        if newinfo not in questiondict:
                            try:
                                questionid = str(sentence['sentenceidx'][0]) + '/' + str(sentence['sentenceidx'][1]) + '/' + str(sentence['sentenceidx'][2]) + '/' + str(sentence['sentenceidx'][3]) + '_' + newinfo
                                questiondict.update({newinfo: {'question': newinfo, 'id': questionid, 'answers': [], 'is_impossible': True}})
                            except TypeError:
                                questionid = str(sentence['datasetidx']) + '/' + str(sentence['textidx']) + '/' + str(sentence['paragraphidx']) + '/' + str(sentence['sentenceidx']) + '_' + newinfo
                                questiondict.update({newinfo: {'question': newinfo, 'id': questionid, 'answers': [], 'is_impossible': True}})
        ...
        currentpath = os.getcwd()
        with open(currentpath + '/Data/SQUAD/SQUADFormat' + datasetlist[datasetidx] + 'V2.json', 'w') as outfile:
...
def sentenceindices(splitsentence, referencedict, language):
    ...
    for reference in referencedict:
        ...
        if referenceword != 'True':
            ...
            if language == 'nl':
                ...
                splitsubsentence = GetSentenceList(referenceword)

            sublistindex = find_sub_list_multiple(splitsubsentence, splitsentence)
            if len(sublistindex) == 0:
                sublistindex = find_sub_list_multiple(splitsubsentence[1:], splitsentence)
            try:
...
def newsplitsentence(splitsentence, sentence):
    newidx = 0
    newsplitsentence = []
    for wordidx, word in enumerate(splitsentence):
        wordsearch = re.search(re.escape(word), sentence[newidx:])
        try:
            wordstart = wordsearch.start()
        except AttributeError:
            print(word)
            print(splitsentence)
            print(sentence)
            print(sentence[newidx:])
            exit(0)
        wordend = wordsearch.end()
        newwordstart = wordstart + newidx
        newwordend = wordend + newidx
        newidx += wordend
        newsplitsentence.append((word, newwordstart, wordidx))
    return(newsplitsentence)

def wordlistidx_prodigy_webnlg(referencedict, splitsentence):
    sentencereplaceindexdict = {}
    for reference in referencedict:
        wordidxlist = []
        for word in splitsentence:
            if (word[1] >= referencedict[reference][4]) and (word[1] < referencedict[reference][5]):
                wordidxlist.append(word[2])

        sentencereplaceindexdict.update({referencedict[reference][2]: wordidxlist})
...
def lexicalizesentence(sentence, referencedict, annotationtype):
    ...
    if annotationtype == 'Prodigy':
        ...
        splitsentence = [re.sub(r'&slash;', '/', x) for x in splitsentence]
        splitsentenceprodigy = newsplitsentence(splitsentence, sentence)

        sentencereplaceindexdict = wordlistidx_prodigy_webnlg(referencedict, splitsentenceprodigy)
    ...
    if annotationtype == 'MyTool':
        ...
        splitsentence = [re.sub(r'&slash;', '/', x) for x in splitsentence]
        sentencereplaceindexdict = sentenceindices(splitsentence, referencedict, 'nl')
...
def templatizesentences(sentence, referencedict, annotationtype):
    if annotationtype == 'MyTool':
        splitsentence = GetSentenceList(sentence)
        #newsentence = ' '.join(newsentence)
        sentencereplaceindexdict = sentenceindices(splitsentence, referencedict, 'en')
                                ...
                                if ';;' in sentence[combineddictkey]:
                                    ...
                                    for content in newsplit:
                                        ...
                                        if len(withattribute) == 0:
                                            withattribute = None
                                            newattribute = content + '__' + newkey
                                        else:
                                            ...
                                            newattribute = [x[1] + '__' + newkey + '__' + str(newkeyidx) for newkeyidx, x in enumerate(withattribute)]
                                        ...
                                        if withattribute == None:
                                            if sentence[combineddictkey] not in referencelist:
                                                ...
                                                combineddictstring = newkey + ' | ' + newsentence
                                                triplesetlist.append(combineddictstring)
                                    ...
                                    newkey = re.sub('\d+', '', combineddictkey)
                                    ...
                                    if len(withattribute) == 0:
                                        withattribute = None
                                        newattribute = sentence[combineddictkey] + '__' + newkey
                                    else:
                                        newattribute = [x[1] + '__' + newkey + '__' + str(newkeyidx) for newkeyidx, x in enumerate(withattribute)]
                                    ...
                                    if withattribute == None:
                                        ...
                                        for naidx, naval in enumerate(newattribute):
                                            if withattribute[naidx][1] + '_' + str(naidx) not in referencelist:
                                                referencedict.update({naval: [entity, refnum, 'PATIENT-' + str(refnum), 'description']})
                                                referencelist.append(withattribute[naidx][1] + '_' + str(naidx))
                                                refnum += 1
            ...
            for paragraph in text:
                ...
                for sentence in paragraph:
                    ...
                    if language == 'nl':
                        ...
                        splitsentenceprodigy = [(token.text, token.idx, token.i) for token in nldoc]
                        splitsentencemytool = [token.text for token in nldoc]
                    elif language == 'en':
                        endoc = ennlp(sentence['sentence'])
                        splitsentenceprodigy = [(token.text, token.idx, token.i) for token in endoc]
                        splitsentencemytool = [token.text for token in endoc]
                    nersentence = ['O'] * len(splitsentencemytool)
                        ...
                        if (combinedkey != 'sentenceidx') and (combinedkey != 'filename') and (combinedkey != 'sentence') and (combinedkey != 'title') and (combinedkey != 'textidx') and (combinedkey != 'paragraphidx') and (combinedkey != 'datasetidx'):
                            if annotationtype == 'Prodigy':
                                for occurrence in sentence[combinedkey]:
                                    sublistindex = wordlistidx_prodigy(occurrence, splitsentenceprodigy)
                                ...
                                if ';;' in sentence[combinedkey]:
                                    ...
                                    for contextidx, content in enumerate(newsplit):
                                        ...
                                        sublistindex = find_sub_list_multiple(splitinfo, splitsentencemytool)
                    ...
                    allsentencelist.append(splitsentencemytool)
