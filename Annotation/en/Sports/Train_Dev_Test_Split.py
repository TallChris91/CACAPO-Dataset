...
englishsports = Convert_Prodigy(currentpath + '/Data/EnglishSportsAll.jsonl', 'en')

trainsample = np.random.choice(np.arange(len(englishsports)), int(round((len(englishsports) / 100) * 76.5, 0)), replace=False)
newrestsample = []
restsample = list(np.arange(len(englishsports)))
for i in restsample:
    if i not in trainsample:
        newrestsample.append(i)

devsample = np.random.choice(newrestsample, int(round((len(englishsports) / 100) * 8.5, 0)), replace=False)
...
newdatalisttrain = np.array(englishsports)[trainsample]
newdatalistdev = np.array(englishsports)[devsample]
newdatalisttest = np.array(englishsports)[testsample]
