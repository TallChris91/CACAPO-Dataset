...
englishstocks = Convert_Prodigy(currentpath + '/Data/EnglishStocksAll.jsonl', 'en')

print(len(englishstocks))
...
filenamesconcatenated = GetFileNames(currentpath + '/Data/EnglishStocksAll.jsonl')

trainsample = np.random.choice(np.arange(len(englishstocks)), int(round((len(englishstocks) / 100) * 76.5, 0)), replace=False)
newrestsample = []
restsample = list(np.arange(len(englishstocks)))
for i in restsample:
    if i not in trainsample:
        newrestsample.append(i)

devsample = np.random.choice(newrestsample, int(round((len(englishstocks) / 100) * 8.5, 0)), replace=False)
...
newdatalisttrain = np.array(englishstocks)[trainsample]
newdatalistdev = np.array(englishstocks)[devsample]
newdatalisttest = np.array(englishstocks)[testsample]
